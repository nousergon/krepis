#!/usr/bin/env bash
# infrastructure/krepis_runner_spot_bootstrap.sh — provision an EC2-spot box,
# register it as an EPHEMERAL GitHub Actions self-hosted runner for
# nousergon/krepis, let it pick up exactly ONE queued job, then
...
#
# Invoked by the `krepis-runner-dispatcher` Lambda over SSM. That
...
#   infrastructure/krepis_runner_spot_bootstrap.sh --job-id <workflow_job.id>
#
# REQUIRED SSM: /krepis/runner/github_pat — fine-grained PAT,
#   owner=nousergon, repo=krepis, Administration: Read and write
...
# REQUIRED AWS: instance profile krepis-runner-executor-profile
#   (infrastructure/iam/krepis-runner-executor-role-{trust,policy}.json) —
...
EXTRA_PYTHON_VERSIONS="${EXTRA_PYTHON_VERSIONS:-3.9 3.10 3.11 3.13}"
NEEDS_NODE_VERSIONS="${NEEDS_NODE_VERSIONS:-}"

KREPIS_RUNNER_JOB_ID=""
while [[ $# -gt 0 ]]; do
  case "$1" in
    --job-id) KREPIS_RUNNER_JOB_ID="$2"; shift 2 ;;
    *) shift ;;
  esac
done
...
log() { echo "[krepis-runner-bootstrap] $*"; }
...
KREPIS_RUNNER_USER="${KREPIS_RUNNER_USER:-ec2-user}"
RUN_USER_HOME="$(getent passwd "$KREPIS_RUNNER_USER" | cut -d: -f6)"
[ -n "$RUN_USER_HOME" ] || RUN_USER_HOME="/home/${KREPIS_RUNNER_USER}"
...
systemd-run --on-active="${MAX_RUNTIME_SECONDS}" --unit=krepis-runner-watchdog \
  --description='krepis-runner spot hard-timeout' /sbin/shutdown -h now \
  >/dev/null 2>&1 || log "WARN: watchdog arm failed (shutdown_behavior=terminate still applies)"
KREPIS_RUNNER_SHUTDOWN_DELAY_SECONDS="${KREPIS_RUNNER_SHUTDOWN_DELAY_SECONDS:-30}"
finish() {
  rc=$?
  log "exit rc=$rc — scheduling shutdown ${KREPIS_RUNNER_SHUTDOWN_DELAY_SECONDS}s from now (non-blocking — this script exits immediately so the SSM agent can report the command's real status before the deferred shutdown fires, config#1472)"
  systemd-run --on-active="${KREPIS_RUNNER_SHUTDOWN_DELAY_SECONDS}" --unit=krepis-runner-delayed-shutdown \
    --description='krepis-runner delayed self-terminate (post-SSM-report)' /sbin/shutdown -h now \
    >/dev/null 2>&1 || {
...
# ── Secrets from SSM (instance profile: krepis-runner-executor-role,
# see infrastructure/iam/krepis-runner-executor-role-{trust,policy}.json — a
# DEDICATED role scoped to exactly one param; see this script's header for why
# it is not a reuse of SATURDAY_SF_WATCH_PAT) ─────────────────────────────────
GH_TOKEN="$(aws ssm get-parameter --name /krepis/runner/github_pat --with-decryption \
  --query 'Parameter.Value' --output text --region "$REGION" 2>/dev/null)"
if [ -z "${GH_TOKEN:-}" ]; then
  log "FATAL: GH_TOKEN missing from SSM (/krepis/runner/github_pat)"
  exit 1
fi
...
RUNNER_NAME="krepis-spot-${INSTANCE_ID:-$(date +%s)}"
...
JIT_CONFIG="$(curl -sS -X POST \
  ...
  "https://api.github.com/repos/nousergon/krepis/actions/runners/generate-jitconfig" \
  -d "$(jq -n --arg name "$RUNNER_NAME" \
    '{name: $name, runner_group_id: 1, labels: ["self-hosted","krepis-spot"], work_folder: "_work"}')" \
  | jq -r '.encoded_jit_config // empty')"
if [ -z "${JIT_CONFIG:-}" ]; then
  log "FATAL: could not mint a JIT runner config — check GH_TOKEN's Administration:write scope on nousergon/krepis"
  exit 1
fi
...
if [ -n "${EXTRA_PYTHON_VERSIONS:-}" ]; then
  ...
  if command -v uv >/dev/null 2>&1; then
    for PYVER in $EXTRA_PYTHON_VERSIONS; do
      runuser -u "$KREPIS_RUNNER_USER" -- env HOME="$RUN_USER_HOME" PATH="/usr/local/bin:${PATH}" \
        uv python install "$PYVER" >/dev/null 2>&1 || { log "WARN: uv python install ${PYVER} failed"; continue; }
      PYBIN="$(runuser -u "$KREPIS_RUNNER_USER" -- env HOME="$RUN_USER_HOME" PATH="/usr/local/bin:${PATH}" \
      ...
      EXTRA_PY_FULL="$(runuser -u "$KREPIS_RUNNER_USER" -- "$PYBIN" -c 'import sys; print(".".join(map(str, sys.version_info[:3])))')"
      ...
      EXTRA_PY_PREFIX="$(runuser -u "$KREPIS_RUNNER_USER" -- "$PYBIN" -c 'import sys; print(sys.prefix)')"
      ...
      curl -sS https://bootstrap.pypa.io/get-pip.py -o /tmp/get-pip-${PYVER}.py \
        && chown "${KREPIS_RUNNER_USER}:${KREPIS_RUNNER_USER}" /tmp/get-pip-${PYVER}.py \
        && runuser -u "$KREPIS_RUNNER_USER" -- "$PYBIN" /tmp/get-pip-${PYVER}.py --break-system-packages --quiet \
...
chown -R "${KREPIS_RUNNER_USER}:${KREPIS_RUNNER_USER}" "$RUNNER_DIR"

log "starting run.sh with JIT config (config#2653) — name=${RUNNER_NAME} job_id=${KREPIS_RUNNER_JOB_ID:-n/a}; no separate config.sh step (--jitconfig writes the settings files directly and implies --ephemeral)"
...
runuser -u "$KREPIS_RUNNER_USER" -- /usr/bin/env HOME="$RUN_USER_HOME" \
